
const blogPosts = [
  {
    id: 1,
    title: "Top 5 Smartwatches for Fitness Enthusiasts in 2024",
    content:
      "Discover the best smartwatches for tracking your fitness goals, from heart rate monitoring to GPS tracking.",
    author: "John Doe",
    date: "2024-03-10",
    image:
      "https://images.unsplash.com/photo-1546868871-7041f2a55e12?q=80&w=600&h=400&auto=format&fit=crop",
    description:
      "Looking for a smartwatch to help you stay on top of your fitness goals? Check out our top 5 picks for 2024!",
    avatar: "https://randomuser.me/api/portraits/men/1.jpg",
  },
  {
    id: 2,
    title: "How to Style Your Wardrobe for Spring 2024",
    content:
      "Learn how to refresh your wardrobe with the latest spring fashion trends, from pastel colors to lightweight fabrics.",
    author: "Jane Smith",
    date: "2024-03-12",
    image:
      "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?q=80&w=600&h=400&auto=format&fit=crop",
    description:
      "Spring is here! Discover the must-have clothing items and styling tips for the season.",
    avatar: "https://randomuser.me/api/portraits/men/2.jpg",
  },
  {
    id: 3,
    title: "The Best Books to Read This Year",
    content:
      "Explore our curated list of the best books of 2024, from gripping thrillers to inspiring self-help guides.",
    author: "Alice Johnson",
    date: "2024-03-15",
    image:
      "https://images.unsplash.com/photo-1512820790803-83ca734da794?q=80&w=600&h=400&auto=format&fit=crop",
    description:
      "Looking for your next great read? Check out our top book recommendations for 2024!",
    avatar: "https://randomuser.me/api/portraits/men/3.jpg",
  },
  {
    id: 4,
    title: "Top 10 Headphones for Music Lovers",
    content:
      "Find the perfect pair of headphones for your music needs, whether you're a casual listener or an audiophile.",
    author: "Mike Brown",
    date: "2024-03-18",
    image:
      "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=600&h=400&auto=format&fit=crop",
    description:
      "From noise-cancelling to wireless, here are the best headphones for every budget and preference.",
    avatar: "https://randomuser.me/api/portraits/men/4.jpg",
  },
  {
    id: 5,
    title: "The Ultimate Guide to Choosing the Right Shoes",
    content:
      "Learn how to pick the perfect shoes for every occasion, from running shoes to formal wear.",
    author: "Sarah Lee",
    date: "2024-03-20",
    image:
      "https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=600&h=400&auto=format&fit=crop",
    description:
      "Struggling to find the right shoes? Our guide will help you make the best choice for comfort and style.",
    avatar: "https://randomuser.me/api/portraits/men/6.jpg",
  },
];

export default blogPosts;
